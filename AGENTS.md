...
- `nextjs-backend/setup.sql` is now the **single source of truth** for the database (all 8 tables + FKs + indexes + RLS enablement). It is hand-maintained — edit it directly. Users apply it by pasting into the Supabase SQL editor or with `npm run db:setup` (`scripts/db-setup.mjs`, runs it over `DATABASE_URL`).
...
- **Schema structure (redesigned 2026-07-20, 11 tables → 7; +`stripe_app_sessions` for the app login flow = 8)** — three groups (rationale in the header of `setup.sql`): Better Auth models (`users`, `sessions`, `auth_accounts`, `verifications` — `users` also carries app-owned `settings` jsonb and the publisher-side `stripe_customer_id_live`/`_test` columns, absorbing the old `billing_customers`); merchant side (`stripe_accounts` — the `acct_` id is the primary key, with account `settings` jsonb and `live_installation_id`/`test_installation_id` install state, absorbing `stripe_app_installations` + `stripe_account_settings`; and `memberships` — users ↔ Stripe accounts many-to-many carrying `role` (owner/admin/member CHECK, first registrant = owner) and per-account `settings`, absorbing `stripe_account_users` + `stripe_account_user_settings`); publisher side (`subscriptions`, keyed by the `sub_` id, formerly `billing_subscriptions`). Conventions: Stripe ids are natural primary keys (no surrogate uuid + unique text pairs); `livemode` survives only where Stripe itself splits data by mode (billing customer columns, `subscriptions.livemode`) — app-owned settings/roles are mode-independent, nest `{"live":…,"test":…}` inside the jsonb if separation is ever needed; `timestamptz` everywhere; every FK column is indexed.
...
Modeled on Parcelcraft's `Login.tsx` + `(login)` routes, converted from Firebase/Redis to Better Auth/Postgres:

- **Handshake:** the app mints a `state` UUID, opens `{backend}/stripe?state=…` in a browser tab, and polls `GET /api/stripe-app/verify?state=…` (signed). The `/stripe` page (session auth) POSTs the state to `/api/stripe-app/session`; verify claims it, writes `stripe_app_sessions` (+ `memberships`, first = owner), and consumes the state (one-shot). `GET /api/stripe-app/userinfo` resolves the signed dashboard identity thereafter; logout = `DELETE /api/stripe-app/session` + the `/stripe-logout` page. Full docs: `nextjs-backend/AUTHENTICATION.md` ("The Stripe App user login flow").
- **Tables:** one new table `stripe_app_sessions` (PK stripe_account_id + stripe_user_id, '' for Connect/platform callers without a `usr_` id). The short-lived handshake states deliberately reuse Better Auth's `verifications` table (identifier `stripe-app-login:<state>`, 15-min TTL) to keep the schema lean — the schema is 8 tables total.
- **Backend pieces:** `src/lib/stripe-app-session.ts` (all DB logic), routes `/api/stripe-app/{session,verify,userinfo}`, `sendResetPassword` in `src/lib/auth.ts` (prints the reset link to the terminal — no email provider wired up), and a `(login)` route group with `/login` (moved, URL unchanged), `/register`, `/reset-password`, `/confirm`, `/stripe`, `/stripe-logout`, `/end-session`, `/complete-setup` sharing one card layout + `components.tsx` form primitives + `redirect.ts` (same-site-only redirect param helpers). The proxy now preserves the query string when bouncing to `/login` (the handshake depends on it) and lists the new public pages.
- **App pieces:** `stripe-app/src/components/Login.tsx` (state machine: initializing → logged-out → waiting-for-auth → logged-in → logging-out, 5s polling), new helpers in `src/api/backend.ts` (`getUserInfo`/`verifyLoginState`/`deleteAppSession`/`loginPageUrl`/`logoutPageUrl`; `BackendConnectionError` now carries `status`), wired into `App.tsx` as demo section 3.
- **Verified 2026-07-20** end-to-end against Supabase project `xcqewnpyuzknxjnximju`, schema `example` (schema was dropped and rebuilt from `setup.sql` — it had stale pre-redesign tables): register → `/stripe` handshake → signed verify/userinfo/logout, plus the password-reset loop.
- **Machine quirk:** this PC had stale *user-scope* Windows env vars (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `supabaseUrl`, `supabaseKey`) pointing at an old Supabase project — process env overrides `.env.local` in Next.js, which broke supabase-js with "Invalid API key". `.claude/launch.json` (gitignored) now launches the dev server through `cmd /c set VAR=&& …` to clear them; the real fix is deleting the four vars from user scope.

## Suggested next session tasks

1. Commit the pending working-tree changes (login flow + schema workflow + auth framework — see `git status`). Commit lockfiles for reproducible installs.
