 ...
 * .env.local is written and the checklist at http://localhost:3006 is green,
...
function buildEnv(v, extras) {
  ...
  // http://localhost:3006 flags them); optional keys stay commented out.
...
#  REPLACE_ME values still need filling in; http://localhost:3006 shows a
...
${req('BETTER_AUTH_URL', 'http://localhost:3006')}
${req('NEXT_PUBLIC_BETTER_AUTH_URL', 'http://localhost:3006')}
...
# For local dev: run \`stripe listen --forward-to localhost:3006/api/stripe/webhook\`
...
`);
  ...
  if (!have('BETTER_AUTH_URL')) v.BETTER_AUTH_URL = 'http://localhost:3006';
  if (!have('NEXT_PUBLIC_BETTER_AUTH_URL')) v.NEXT_PUBLIC_BETTER_AUTH_URL = 'http://localhost:3006';
...
`);
  ...
  if (have('SUPABASE_SCHEMA')) {
    ...
    if (pick === 1) {
      ...
      console.log(
        yellow(
          `  One manual step: in the Supabase dashboard, open Project Settings → Data API  →  Settings Tab  and add\n  "${v.SUPABASE_SCHEMA}" to "Exposed schemas" — the backend can't query it until then.`,
        ),
      );
    }
  }
  ...
  console.log(`
  Start everything with ${cyan('npm run dev')} — ${cyan('http://localhost:3006')} shows a live
