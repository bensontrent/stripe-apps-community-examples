...
- `nextjs-backend/setup.sql` is a **generated** file (all 11 tables + FKs) that users can paste into the Supabase SQL editor instead of running `db:push`. Do not edit it by hand.
...
- Migrations were reset again on 2026-07-05 during the schema restructure (now a single `0000_pink_misty_knight.sql`) — safe because the DB was still empty. **Once real data exists in Supabase, migration resets are off the table.** (`drizzle/` is gitignored; only the generated `setup.sql` is tracked.)
- **Schema restructured 2026-07-05** around three prefixes (rationale documented at the top of `schema.ts`): `auth_*` = Better Auth models; `stripe_*` = merchant side — `stripe_accounts` (one row per `acct_` id), `stripe_account_users` (users ↔ Stripe accounts many-to-many), and per-`livemode` child tables `stripe_app_installations` / `stripe_account_settings` / `stripe_account_user_settings` so live and test settings never bleed; `billing_*` = publisher-side monetization (`billing_customers` one per user per livemode, `billing_subscriptions`). The old `app_installations` (tied an account to one user), `stripe_customers`, and `stripe_subscriptions` are gone.
- **Better Auth mapping is non-default:** `src/lib/auth.ts` maps Better Auth's `user`/`session`/`account`/`verification` models onto `users`/`sessions`/`auth_accounts`/`verifications` with `uuid` ids (`generateId: () => crypto.randomUUID()`). Better Auth's own defaults are singular names + text ids — that's what the deleted `auth-schema.ts` contained. `auth_accounts` is deliberately not called `accounts` to avoid confusion with Stripe accounts. When changing the four auth tables, keep the columns Better Auth expects and don't regenerate/wire in the CLI's schema file.
